...
import sys
import os
from models_yahoo import Encoder, Decoder
...
for epo in torch.arange(epo_0 + 1, args.num_epochs + 1):
    ...
    for i, bat in enumerate(pbar):
        ...
        rec = sum([criterion(preds[:, l], sents[:, l + 1]) for l in range(preds.size(1))])
        loss = rec + beta * KL.mean(-1)

        if i % 10 == 0:
            print(f'KL: {KL.mean(-1).item():.4f} | Rec: {rec:.4f}')
