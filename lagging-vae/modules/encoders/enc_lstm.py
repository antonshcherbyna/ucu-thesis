...
class LSTMEncoder(GaussianEncoderBase):
    ...
    def __init__(self, args, vocab_size, model_init, emb_init, flow=None, num_flows=1, flow_type='planar'):
        ...
        if flow is not None:
                self.flow = NormalizingFlow(args.nz, num_flows, flow_type=flow_type)
        else:
            self.flow = None
