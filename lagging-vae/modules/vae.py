...
class VAE(nn.Module):
    ...
    def __init__(self, encoder, decoder, flow, args):
    ...
    def nll_iw(self, x, nsamples, ns=100):
        ...
        for _ in range(int(nsamples / ns)):
            ...
            #SLDJs = torch.zeros(1).to(z.device)
            z, z0, log_det, param = self.encoder.sample(x, ns)

            # [batch, ns]
            if self.flow is None:
                log_comp_ll = self.eval_complete_ll(x, z, None)
                log_infer_ll = self.eval_inference_dist(x, z, param)
            else:
                log_comp_ll = self.eval_complete_ll(x, z, z0)
                log_infer_ll = self.eval_inference_dist(x, z0, param) + log_det.squeeze(-1)
    ...
    def eval_complete_ll(self, x, z, z0):
        """compute log p(z,x)
        Args:
            x: Tensor
        ...
        if self.flow is not None:
            log_prior = self.eval_prior_dist(z0)
        else:
    ...
    def sample_from_inference(self, x, nsamples=1):
        ...
        """
        z, _, _, _ = self.encoder.sample(x, nsamples)
    ...
    def calc_mi_q(self, x):
        ...
        """
        #if self.flow:
        #    return self.encoder.calc_mi_flow(x)
        #else:
