...
def init_config():
    ...
    id_ = "%s_aggressive%d_kls%.2f_warm_%d_flow_%d_flow_type_%s_num_flows_%d_%d_%d_%d" % \
            (args.dataset, args.aggressive, args.kl_start,
             args.warm_up, args.flow, args.flow_type, args.num_flows, args.jobid, args.taskid, args.seed)
...
def main(args):
    ...
    if args.enc_type == 'lstm':
        if args.flow == True:
            encoder = LSTMEncoder(args, vocab_size, model_init, emb_init, flow=args.flow, num_flows=args.num_flows, flow_type=args.flow_type)
        else:
            encoder = LSTMEncoder(args, vocab_size, model_init, emb_init, flow=None, num_flows=args.num_flows, flow_type=args.flow_type)
    ...
    if args.flow == True:
        vae = VAE(encoder, decoder, args.flow, args).to(device)
    else:
        vae = VAE(encoder, decoder, None, args).to(device)
